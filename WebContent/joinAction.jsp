...
<body>
	<%
		...
		if (user.getId() == null || user.getPasswd() == null || user.getEmail() == null || user.getPhone() == null
				|| user.getName() == null) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			...
			if (userDAO.avoidDupID(user.getId()) != null) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
				script.println("alert('이미 존재하는 아이디입니다')");
				script.println("history.back()");
				script.println("</script>");
			} else if (userDAO.avoidDupName(user.getName()) != null) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
