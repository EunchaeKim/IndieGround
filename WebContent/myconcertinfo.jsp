...
<body>
	...
	<div class="col-md-2">
		...
		<p class="text-center">
			<a class="btn btn-default" data-target="#modal" data-toggle="modal">프로필
				보기</a>
		</p>
		<p class="text-center">
...
<div class="col-md-8">
	...
	<h4>내가 기획한 공연</h4>
		<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
			...
			<%
			ArrayList<Bbs> myconcertList=bbsDAO.getmyConcert(userID);
				if(myconcertList!=null){
				for(int i=0 ; i<myconcertList.size() ; i++){
					%>
				<tr>
					<td><a href="view.jsp?bbsID=<%= myconcertList.get(i).getBbsID() %>"><%= myconcertList.get(i).getBbsTitle().replaceAll(" ","&nbsp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll("\n","<br>") %></a></td>
					<td><%= myconcertList.get(i).getBbsDate()%></td>
					<td><%= myconcertList.get(i).getBbsLocation()%></td>
					<td><%= myconcertList.get(i).getBbsDeadline()%></td>
					<td><%= myconcertList.get(i).getBbsConcertday()%></td>
				</tr>
		<%
				}
				}
